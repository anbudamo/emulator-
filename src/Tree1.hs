...
updateRoot item (Folder rootItem children) = Folder item children
updateRoot item (File rootItem) = File item
...
countFlatTree (entry:entries) (numDirs, numFiles) = 
    case entry of 
        (Folder name []) -> countFlatTree entries (numDirs+1, numFiles)
        (File name) -> countFlatTree entries (numDirs, numFiles)
